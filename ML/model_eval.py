...
class DataImport:
    ...
    def import_data(self, data_path):
        ...
        filenames = [f for f in filenames if not any(s in os.path.basename(f) for s in exclude)]
        ...
        #dataframes = [pd.read_csv(f, engine="pyarrow") for f in filenames]
...
def optuna_objective(data, args, trial, **kwargs):
    ...
    for fold, (train_index, test_index) in enumerate(data.cross_val_iter):
        ...
        if args.augment:
            ...
            if not train_data:
                raise ValueError("ERROR: build_augmented_dataset returned an empty list")
